...
export default function CooperadoFormModal({ open, onClose, onSubmit, initialData }) {
    ...
    const cpfCnpjMask = (value) =>
        value.replace(
            ...
            (_, p1, p2, p3, p4) =>
                p4.length === 4 ? `${p1}.${p2}.${p3}/${p4}` : `${p1}.${p2}.${p3}-${p4}`
        ).slice(0, 14);
    ...
    return (
        <Dialog open={open} onClose={handleClose} maxWidth="sm" fullWidth>
            <DialogTitle>{initialData ? "Editar Cooperado" : "Novo Cooperado"}</DialogTitle>
            <DialogContent dividers>
                <Grid container spacing={2}>
                    {[
                        { label: "Nome", name: "nome", type: "text" },
                        { label: "CPF/CNPJ", name: "cpf_cnpj", type: "text", maskFn: cpfCnpjMask },
                        {
                        ...
                        { label: "Renda/Faturamento", name: "renda_faturamento", type: "number" },
                        { label: "Telefone", name: "telefone", type: "text", maskFn: phoneMask },
